...
import { Cell2, Direction } from '../models/cell2';
...
describe.only('Game Play', () => {
    ...
    // position ships
    bs.placeShip(player1, player1Ships[0],  new Cell2(0,0));
    bs.placeShip(player1, player1Ships[1],  new Cell2(1,0));

    bs.placeShip(player2, player2Ships[0],  new Cell2(4,0));
    bs.placeShip(player2, player2Ships[1],  new Cell2(0,1));

    it('Player 1 attacks Player 2 - Miss', () => {
        expect(bs.attackPlayer(player1, player2, new Cell2(0,0))).to.equal(false);
    });

    it('Player 2 attacks Player 1 ship (orientation: horizontal, size 3)', () => {

        // p2 hits p1 ship
        expect(bs.attackPlayer(player2, player1, new Cell2(0,0))).to.equal(true);
        ...
        // p2 hits ship
        expect(bs.attackPlayer(player2, player1, new Cell2(0,1))).to.equal(true);

        // damaged but not yet destroyed
        expect(player2Ships[0].isDestroyed()).to.equal(false);

        // p2 hits ship
        expect(bs.attackPlayer(player2, player1, new Cell2(0,2))).to.equal(true);
    ...
    it('Player 2 Wins - destroys all player 1 ships', () => {
        ...
        expect(bs.attackPlayer(player2, player1, new Cell2(1,0))).to.equal(true);
        expect(bs.attackPlayer(player2, player1, new Cell2(1,1))).to.equal(true);
        expect(bs.attackPlayer(player2, player1, new Cell2(1,2))).to.equal(true);
        expect(bs.attackPlayer(player2, player1, new Cell2(1,3))).to.equal(true);
        expect(bs.attackPlayer(player2, player1, new Cell2(1,4))).to.equal(true);
